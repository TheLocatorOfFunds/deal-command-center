-- Three new review rules (Nathan 2026-06-23), tuned against real briefs:
--  dnc            — a linked contact is do_not_call/do_not_text (hard don't-dial)
--  self_claim_risk— homeowner already acting (filed pro se / retained attorney /
--                   filed a surplus petition). Dropped "another locator" matches
--                   (that's the AI urging speed on a HOT lead, not a kill).
--  other_liens    — junior/senior lien or 2nd mortgage that may CONSUME the
--                   surplus. Requires a consuming context so "no junior lien" /
--                   "minimal senior liens" don't false-fire.
-- Applied to prod via MCP apply_migration. Supersedes 20260623120000.
...
with base as (
  select d.id, d.name, d.status, d.meta,
    ...
    (exists (select 1 from contact_deals cd join contacts c on c.id = cd.contact_id where cd.deal_id = d.id and (c.do_not_call or c.do_not_text))) as has_dnc,
