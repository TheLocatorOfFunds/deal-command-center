...
|---|---|---|---|
| **2026-05-26** | Justin | `claude/mobile-feature-parity-p3pPZ` · issue #210 | Pivoted Vapi voice-agent integration from "Vapi-attached Claude" to "Vapi-as-transport, Lauren-as-brain" via Custom LLM endpoint. New `_shared/lauren-brain.ts` (SYSTEM + VOICE_ADDENDUM + safety filters + caller-context resolver) used by new `lauren-voice` Edge Function — Anthropic↔OpenAI streaming translation, auto-resolves caller phone → case context on every turn. Voicemail fallback chained after Vapi SIP `<Dial>` so a Vapi outage degrades to existing voicemail in 10s. Deleted `vapi-lookup-deal` (subsumed). New GH Action `deploy-functions.yml` (workflow_dispatch + SUPABASE_PAT) + `npm run vapi-create-assistant` script to drive dashboard-free setup. Runbook at `docs/VAPI_SETUP.md`. → [archive](./2026-05-26-vapi-lauren-pivot.md) |
