...
|---|---|---|---|
| **2026-06-04** | Justin | `justin/outbound-calling` (OTA) | Outbound calling shipped on the iPhone app via EAS Update — SDK dial, no modal, navy in-call screen suppressed for outbound (`isOutboundCallActive()` guard), navigate DCC freely mid-call; verified on-device + `call_logs`. Three durable gotchas: an OTA only reaches a build if published *after* that build (4 earlier OTAs were dead because Build 26 postdated them); `EXPO_TOKEN` was wrapped in literal `<>` → "bearer token invalid" (strip with `${EXPO_TOKEN//[<>]/}`); use homebrew `eas` not `npx eas-cli@latest`. Two parallel Justin sessions race native builds to the same `preview` channel — next build MUST come from `justin/outbound-calling` or it orphans the OTA. → [archive](./2026-06-04-outbound-calling-ota.md) |
