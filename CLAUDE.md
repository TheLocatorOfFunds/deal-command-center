...
- **New UI dropdown/enum value → check the DB CHECK constraint / enum FIRST.** (2026-06-30) Eric's "Other doesn't save" was a `call_logs_outcome_check` that only allowed the original 5 outcome codes; the 4 added this session (`other`/`booked`/`not_interested`/`do_not_call`) violated it, so every write of those silently failed. **When you add a value to any UI picker that writes a constrained column (`call_logs.outcome`, `deals.status`, `deals.type`, `call_logs.status`/`direction`, etc.), widen the column's CHECK/enum in the SAME change.** Find them with `select conname, pg_get_constraintdef(oid) from pg_constraint where conrelid='public.<table>'::regclass and contype='c';`
- **Never write to Supabase without checking the result.** A bare `await sb.from(t).update(x).eq(...)` swallows BOTH constraint violations AND RLS 0-row blocks — no throw, no log, the optimistic UI just closes. That single missing `.select()` is what hid the bug above for everyone, not just Eric. Pattern: `.select('id')`, then `if (error) throw error; if (!rows?.length) throw new Error(...)`. Applies to insert/update/delete on any write the user expects to persist.
- **⏸ On-hold leads (`meta.hold`) — "not dead, just don't call right now."** (2026-08-04, Colleen Doherty: loss mitigation approved + court stay.) Shape: `{reason, until, by, at}`; `dealHold()` in `src/app.jsx` is the ONE gate — it returns null once the optional `until` date passes, so expired holds walk back into the queues on their own. Set/cleared via the ⏸ button in the deal header (HoldModal); badge renders through `DealStatusBadges`. Every calling surface must exclude `dealHold(d)` leads — currently Power Dialer, Call Queue (they get their own `⏸ On hold` chip there), FreshAuctionStrip, DailyWorklist (deadlines still show), My Day. **If you build a new outreach/queue surface, add the `dealHold` filter — and if automations ever un-pause, `generate-outreach` needs the same guard server-side (it does NOT have one today).** Not an intel-managed meta key; the sync never touches it. A hold is NOT a kill: no dispositionReason, no kill-gate involvement.
- **Never use `<input type="number">` for money fields — pasting wipes the value.** (2026-07-08, Inaam's "fields disappear when I save.") Pasting a formatted figure like `$140,430` into a number input puts the browser in badInput state: `e.target.value` becomes `""`, and our `"" ? null : parseFloat` handlers saved **null** — silently WIPING the field (nulled `estimatedSurplus` on 33 of 38 worked July leads). Use the shared **`MoneyInput`** component in `src/app.jsx` (text + `inputMode="decimal"` + `$ ,`-stripping sanitizer; commits null only on deliberate clear, never on unparseable input). Also: money DISPLAY should read the full cascade (`meta.verifiedSurplus → meta.estimatedSurplus → meta.estimated_surplus → meta.estimatedAvailableEquity → deals.surplus_estimate` column) — see `surplusPhase()` — because the same figure historically lives under sibling keys.
